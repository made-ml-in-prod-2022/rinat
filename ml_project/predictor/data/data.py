...
class DataPreprocessor:
    ...
    def data_loader(self) -> pd.DataFrame:
        ...
        logger.debug(
            f'Table has shape {df.shape}'
        )
        return df

    def data_splitter(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        logger.info(f'Split dataset into train/test sizes: {1 - self.test_size}/{self.test_size}')
        train_df, valid_df = train_test_split(df, test_size=self.test_size, shuffle=self.shuffle)
